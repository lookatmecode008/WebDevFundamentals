...
function renderSection(activeSect) {
  ...
  contentArray.forEach((contentArray) => {
    contentArray.classList.remove("active");
  }); //removes .active class from each item in the array
  ...
  if (activeSect === "Pro") {
    ...
    contentArray[3].classList.add("active");
  } //displays welcome card as a default
}
...
function displaySection(buttonPressed) {
  ...
  quirksArray.forEach((quirksArray) => {
    quirksArray.classList.remove("active");
  });
